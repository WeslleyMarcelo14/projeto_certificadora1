...
const NovaDoacaoPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("/api/estoque", {
        ...
        body: JSON.stringify({ productId, quantity: Number(quantity), notes }),
      });

      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(errData.error || "Erro ao cadastrar doação");
      }

      toast({ description: "Doação cadastrada com sucesso!" });
      router.push("/estoque");
    } catch (err) {
      toast({ description: err.message });
    } finally {
  ...
  return (
    <ContentLayout title="Nova Doação">
